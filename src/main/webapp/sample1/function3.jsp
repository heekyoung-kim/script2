...
<script type="text/javascript">
	/*
	  ...
	  3. 화살표 함수(함수표현식을 더 단순한고 간결한 문법으로 저으이하는 것)
	  	var 변수 = (매개변수, 매개변수, 매개변수) => 수행문. 
	  	function이라는 기호가 없어지는 것.
	  	var 변수 = (매개변수,매개변수,매개변수) => {
	  		수행문;
	  		수행문;
	  	}
	  	*화살표 함수의 특징 (자바8부터 제공되는 람다식과 똑같은 형식 but, 사용법이 약간 다름.)
	  		+ 매개변수가 없는 화살표 함수는 아래와 같이 작성할 수 있다.
	  			var 변수 = () => 수행문
	  		+ 매개변수가 하나만 있는 화살표함수는 매개변수를 감싸는 괄호를 생략할 수있다.
	  			var 변수 = 매개변수 => 수행문
	  			var 변수 = (매개변수) => 수행문
	  	...
	  	함수는 위의 방법 중 어떤 방식이 좋은가?
	  		+ 첫번째 고려대상은 언제나 함수선언이다.
	  			...
	  			* 함수의 이름을 통해서 어떤 기능이 정의된함수인지 빠르게 파악할 수있다.
	  		...
	  		+ 함수로 구현한 기능이 딱 한군데에서만 사용되거나 혹은 다른 함수의 매개변수로 전달되는 함수인 경우 함수 표현식을 고려할 수 있다.
	  			...
	  			* 함수를 호출할 때 그 함수의 매개변수로 함수가 전달되는 경우, 전달되는 함수를 함수 표현식으로 작성하는 것이 좋다.
	...
	*/
	...
	function 계산기(a, b, fn){
	 ...
	 // 화살표함수는 매우 간단한 경우 사용.
	 // 다른함수에 매개변수로 전달되는데 기능이 간단한 경우 사용.
	 계산기(10, 20, (a, b) => a - b);
	 계산기(10, 20, (a, b) => {a - b});
</script>
</body>
